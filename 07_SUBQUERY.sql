...
-- 전 직원의 급여 평균보다 많은(초과) 급여를 받는 직우너의
...
-- FROM, JOIN 으로 인해 테이블상 존재하는 컬럼이라면
-- ORDER BY 절 사용가능!
...
-- 메인쿼리 + 서브쿼리
SELECT EMP_NO, EMP_NAME, DEPT_CODE, SALARY, HIRE_DATE
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
...
-- 2) 부서별 급여 합이 17,700,000 인 부서의 부서명과 급여합 조회
...
-- 2. 다중형 서브쿼리(MULTI ROW SUBQUERY)
-- 서브쿼리의 조회 결과 값의 개수가 여러행일때

/*
 * >> 다중형 서브쿼리 앞에는 일반 비교연산자 사용불가
 * 
 * - IN / NOT IN : 여러개의 결과값 중에서 한개라도 일치하는 결과값이 있다면 혹은 없다면 이라는 의미
 * 
 * -> ANY / < ANY : 여러개의 결과값 중에서 한 개라도 큰 / 작은 경우
 * 									-> 가장 작은값 보다 큰가 / 가장 큰 값 보다 작은가?
 * -> ALL , < ALL : 여러개의 결과값의 모든 값 보다 큰 / 작은 경우
 * 									-> 가장 큰 값 보다 큰 갑 / 	가장 작은 값 보다 작은가?
 * - EXISTS / NOT EXISTS : 값이 존재하는 가? / 존재하지 않는가?
 * 
 * */
...
-- 2. 선택함수 사용
--> DECODE();
--> CASE WHEN 조건 1 THEN 값 1 ... ELSE END
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME ,
...
--1) 대리인 직원들의 사번, 이름, 직급명, 급여 조회
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리';

-- 2) 직급이 과장인 직원들의 급여 조회 (서브 쿼리)
SELECT SALARY
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME = '과장';
...
-- > ALL , < ALL : 가장 큰 값보다 크냐? / 가장 작은값 보다 작냐?
...
-- LOCATION 테이블에서 NATIONAL_CODE 가 KO 인 경우의 L0CAL_CODE 와
-- DEPARTMENT 테이블의 LOCATION_ID와 동일한 DEPT_ID가
-- EMPLOYEE 테이블의 DEPT_CODE 와 동일한 사원 조회해라

-- 1) LOCATION 테이블에서 NATIONAL_CODE 가 KO 인 경우의 LOCAL_CODE
SELECT LOCAL_CODE
FROM LOCATION
WHERE NATIONAL_CODE = 'KO'; -- L1(단일 행 서브쿼리)

--2) DEPARTMENT 테이블의 위의 결과 (L1)와 동일한 LOCATION_ID를 가진 DEPT_ID
SELECT DEPT_ID
FROM DEPARTMENT
...
-- 3) 최종적으로 EMPLOYEE 테이블에서 위의 결과들과 동일한 DEPT_CODE 를 가진 사원 조회해라
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
------------------------연습문제---------------------------------------------
--1. 노웅철 사원과 같은 부서, 같은 직급인 사원을 조회 (단, 노웅철 제외)
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
                                ...
                                /*TO_DATE(HIRE_DATE, 'YYYY-MM-DD') > TO_DATE('2000-01-01', 'YYYY-MM-DD') 
                                  AND TO_DATE(HIRE_DATE, 'YYYY-MM-DD') < TO_DATE('2010-01-01', 'YYYY-MM-DD'));*/
--3. 77년생 여자 사원과 동일한 부서이면서, 동일한 사수를 가지고 있는 사원 조회
-- 사번, 이름, 부서코드, 사수번호, 주민번호, 입사일

SELECT EMP_NO, EMP_NAME, DEPT_CODE, MANAGER_ID, HIRE_DATE
FROM EMPLOYEE
...
-- 5. 상[호연]관 서브쿼리
-- 상관 쿼리는 메인 쿼리가 사용하는 테이블값을 서브쿼리가 이용해서 결과를 만듬
-- 메인쿼리의 테이블 값이 변경되면 서브쿼리의 결과값도 바뀌게 되는 구조

-- 상관쿼리는 먼저 메인쿼리 한 행을 조회하고

-- 해당행이 서브쿼리의 조건을 충족하는지 확인하여 SELECT 를 진행함
...
--> 상관 서브쿼리를 사용하여 각 직원의 MANAGER)ID가
--> 실제로 직원 테이블의 EMP_ID와 일치하는지 확인.

-- 메인 쿼리 (직원의 사번, 이름, 부서명, 사수 사번 조회)
...
-- SQL에서 단일값 '스칼라' 라고 함
-- SELECT 절에 작성되는 단일행 단일열 서브쿼리를 스칼라 서브쿼리라고 함

-- 모든 직원의 이름, 직급, 급여, 전체 사원중 가장 높은 급여와의 차를 조회
...
SELECT EMP_NO, EMP_NAME, NVL(MANAGER_ID, '없음') "관리자 사번",
NVL((SELECT EMP_NAME FROM EMPLOYEE SUB WHERE SUB.EMP_ID = MAIN.MANAGER_ID), '없음') "관리자 명"
FROM EMPLOYEE MAIN;

--7. 인라인 뷰(INLINE-VIEW)
-- FROM 절에서 서브 쿼리를 사용하는 경우로
-- 서브쿼리가 만든 결과의 집합(RESULT SET)을 테이블 대신 사용하는것
...
--> 2) 메인쿼리 조회시 ROWNUM 을 5 이하까지만 조회
SELECT ROWNUM, EMP_NAME, SALARY
FROM(SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC) -- 해석순위 1순위 FROM 절에서 전체 직원의 SALARY 를 오름차순으로 정렬
WHERE ROWNUM <= 5; -- 해석순위 2순위 WHERE 절에서 가상행의 1~5행 까지만 조회
...
FROM (SELECT DEPT_CODE, DEPT_TITLE, CEIL(AVG(SALARY)) AS "평균급여" 
			FROM EMPLOYEE
			LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- EX) 공동 1위가 2명이면 다음순위가 2위가 아닌 3위로 
...
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE DEPT_CODE = (SELECT DEPT_CODE FROM EMPLOYEE WHERE EMP_NAME = '전지연') AND
NOT EMP_NAME = '전지연';
...
SELECT EMP_NO, EMP_NAME, DEPT_CODE, MANAGER_ID, EMP_NO, HIRE_DATE
FROM EMPLOYEE
WHERE (DEPT_CODE, MANAGER_ID) IN (SELECT DEPT_CODE, MANAGER_ID FROM EMPLOYEE WHERE SUBSTR(EMP_NO, 1, 2) = '77');
...
SELECT
    EMP_ID,
    EMP_NAME,
    J.JOB_NAME,  -- JOB 테이블의 JOB_NAME 사용
    EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) AS "나이",
    TO_CHAR((SALARY + (SALARY * NVL(BONUS, 0))) * 12, 'L999,999,999') AS "보너스 포함 연봉"
FROM EMPLOYEE E  -- EMPLOYEE 테이블에 별칭 E 부여
JOIN JOB J ON E.JOB_CODE = J.JOB_CODE  -- EMPLOYEE 테이블과 JOB 테이블을 JOB_CODE로 JOIN
WHERE (E.JOB_CODE, TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) IN (
    SELECT JOB_CODE, MIN(TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'))
    FROM EMPLOYEE
    GROUP BY JOB_CODE
)
ORDER BY "나이" DESC;
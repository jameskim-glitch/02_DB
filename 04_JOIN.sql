...
ON(DEPT_CODE = DEPT_ID); -- 23행(DEPT_CODE가 NULL 인 하동운, 이오리 포함)
...
-- 3) FULL[OUTER] JOIN : 합치기에 사용한 두 테이블이 가진 모든 형을 결과에 포함
-- ** 오라클 구문에는 FULL OUTER JOIN 없음
...
--> JOIN 구문 잘못 작성되는 경우 CROSS JOIN 의 결과가 조회됨
...
--> 없는데도 자연조인을 이용할 경우 교차조인 결과가 조인됨
...
AND LOCATION_ID = LOCAL_CODE; -- (EMPLOYEE )
...
SELECT EMP_NAME, JOB_NAME, DEPT_TITLE, SALARY, (SALARY * (1 + NVL(BONUS, 0))) AS "연봉(보너스포함)"
FROM EMPLOYEE E
...
FROM EMPLOYEE E1
JOIN EMPLOYEE E2 ON (E1.DEPT_CODE = E2.DEPT_CODE, E1.EMP_ID != E2.EMP_ID)

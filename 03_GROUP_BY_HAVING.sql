 ...
 * 6. : ORDER BY 컬럼명 | 별칭 | 컬럼순번 정렬방식(ASC/DESC) [NULL FIRST | LAST];
 * 
 */
...
-- 그룹으로 묶은값에 대해서 SELECT 절에서 그룹함수를 사용함

-- 그룹함수는 단 한개의 결과값만 산출하기 때문에 그룹이 여러개일 경우 오류 발생
-- 여러개의 결과값을 산출하기 위해서 그룹함수가 적용된 그룹의 기준을 ORDER BY 절에 기술하여 사용

-- EMPOLYEE 테이블에서 부서코드, 부서별 합 조회
...
-- 직급코드가 같은 사람의(직급을 그룹으로 묶기 : JOB CODE)
...
-- EMPLOYEE 테이블에서
-- 성별(남/여) 과 각 성별 별 인원수, 급여합을
-- 인원 수 오름차순으로 조회
/*3*/SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') AS "성별", COUNT(*) AS "인원 수", SUM(SALARY) AS "급여합"
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용 x (SELECT 보다 먼저 해석)
/*4*/ORDER BY "인원 수"; -- 별칭 사용 O (SELECT 절 해석 완료)
...
GROUP BY DEPT_CODE; -- WHERE 절에서 구한 부서코드가 D5,D6 인 데이터를 그룹으로 묶음
...
-- * 여러 컬럼을 묶어서 그룹으로 지정가능 --> 그룹 내 그룹이 가능하다! *
...
-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서별로 같은 직급인 사원의 인원수를 조회 (GROUP BY 절 DEPT_CODE, JOB_CODE 묶음)
...
FROM EMPLOYEE
--WHERE SALARY >= 3000000 --> 한 사람의 급여가 300만원 이상이라는 조건
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹중 급여 평균이 300만원 이상인 그룹만 조회
ORDER BY DEPT_CODE;
 ...
 * 집합 연산자 (SET OPERATOR)
 * 
 * -- 여러 SELECT 의 결과 (RESULT SET) 를 하나의 결과로 만드는 연산자
 * 
...
-- EMPLOYEE 테이블에서
-- (1번째 SELECT 문)부서 코드가 'D5'인 사람의 사번, 이름, 부서코드, 급여 조회
...
-- (주의 사항!) 집합연산자를 사용하기 위한 SELECT 문 들은

...
--like : 비교하여는 값이 특정한 패턴을 만족시키면 조회하는 연산자
-- [작성법]
...
-- EMPLOYEE 테이블에서 EMAIL의 _앞에 글자가 세글자인
-- 이름, 이메일 조회
...
-- EMPLOYEE 테이블에서
-- 이메일 '_'앞이 4글자 이면서
-- 부서코드가 'D9' 또는 'D6'이고 --> AND 가 OR 보다 우선순위 높다, () 사용 가능
...
----------------------------------------------------
/* ORDER BY 절
 * - SELECT 문의 조회결과(RESULT SET)를 정렬할 때 사용하는 구문
 * 
 * ** SELELCT 문 해석 시 가장 마지막에 해석된다!!
 * */
...
FROM EMPLOYEE
ORDER BY SALARY ALTER ; -- ASC(오름차순) 가 기본값 [ASC | DESC]
-- 컬럼 명 사용.

...
-- 사번, 이름, 부서명, 직급명 조회 하기 위한 VIEW 생성
--> OR REPLACE 옵션을 사용
...
-- VIEW 를 생상할 때 사용된 컬럼에만 데이터가 삽입되었고
-- 사용되지 않은 컬럼(DEPT_TITLE)에는 NULL 이 들어감
-- NULL 은 DB 의 무결성을 약하게 만드는 주요 원인
-- 가능하면 의도하지 않은 NULL 은 존재하지 않게 해야한다.

/* 무결성 : 데이터 베이스에서 데이터를 정확하고 일관되게 유지하기 위한 중요한 개념
 * 
...
--> VIEW 를 이용해서 DML(INSERT/UPDATE/DELETE) 막기 위해서
...
INCREMENT BY 5 -- NEXTVAL 5씩 증가(음수도 가능은 함)
MAXVALUE 150 -- 증가 가능한 최대 값
NOMINVALUE -- 최소값 없음
NOCYCLE -- 반복(순환) 안함
...
--08004: 시퀀스 SEQ_TEST_NO.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다
...
-- SEQ TEST_VALUE 의 MAXVALUE 값을 200 으로 수정
...
-- 변수, 조건문(IF/CASE), 반복문(LOOP, WHILE) 사용가능

...
-- DEPT-COPY 의 DEPT_TITLE 컬럼에 UNIQUE 추가
ALTER TABLE DEPT_COPY 
ADD CONSTRAINT DEPT_COPY_TITLE_U UNIQUE(DEPT_TITLE);

-- DEPT_COPY 에서 DEPT_TITLE 컬럼에 UNIQUE 삭제
ALTER TABLE DEPT_COPY 
...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; -- DEPT_TITLE 컬럼에 NOT_NULL 적용
...
-- 컬럼이 생성되면서 DEFAULT 값이 자동으로 생성되어 있음
...
--> DEPT_ID 의 데이터 타입이 CHAR(2) 이므로 영어+숫자 2글자 까지 저장가능
--> D10 은 3바이트 , CHAR(2) 에 들어갈 수 없ㅇ므
--> VARCHAR2(3) 으로 변경해보기(남는 바이트 메모리 반환 위해)

-- DEPT_ID 컬럼 데이터 타입 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3);
--> 컬럼 데이터 타입 수정후 다시 위 INSERT 실행 -> 삽입 성공 확인!
...
-- DEPT_COPY 테이블에 PRIMARY KEY 추가
--(컬럼 : DEPT_ID, 제약 조건명 D_COPY_PK)

ALTER TABLE DEPT_COPY ADD CONSTRAINT D_COPY_PK 
PRIMARY KEY (DEPT_ID);

-- 3. 이름 변경(컬럼명, 테이블명, 제약조건명)

-- 1) 컬럼 명 변경 (DEPT_TITLE -> DEPT_NAME)
ALTER TABLE DEPT_COPY 
RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

SELECT * FROM DEPT_COPY; 

-- 2) 제약 조건명 변경(D_COPY_PK -> DEPT_COPY_PK)
ALTER TABLE DEPT_COPY RENAME CONSTRAINT D_COPY_PK TO DEPT_COPY_PK;

-- 3) 테이블명 변경 (DEPT_COPY -> DCOPY)
ALTER TABLE DEPT_COPY RENAME TO DCOPY;

SELECT * FROM DEPT_COPY; -- 오류 (테이블 명 변경)
...
DROP TABLE TB1 CASCADE CONSTRAINTS; -- 테이블 삭제
...
--> DDLE 은 수행시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT 시킴
--> DDL 이 종료된 후 DML 구문을 수행할 수 있도록  권장!
...
SELECT * FROM TB2;
-- ROLLBACK 안된다 위에서 DDL 구문중 ALTER 를 사용해서

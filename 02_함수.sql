...
-- 단일행 함수 : N개의 값을 읽어서 연산 후 N 개의 결과를 반환

-- 그룹 함수 : N 개의 값을 읽어서 연산 후 1개의 결과를 반환(합계, 평균, 최대, 최소)

-- 함수는 SELECT 문의
-- SELECT절, WHERE절, ORDER BY절, GROUP BY절, HAVING 절 사용가능
...
-- 컬럼이나 문자열에서 지정한 위치부터 지정된 길이 만큼 문자열을 잘라내어 반환
-- 잘라낼 길이 생략시 끝까지 잘라냄
...
SELECT TRUNC(123.456, 1) FROM DUAL; -- 123.4 , 소수점 첫째자리 아래를 절삭
SELECT TRUNC(123.456, -1) FROM DUAL; -- 120 , 십의자리 아래를 절삭
...
SELECT SYSDATE FROM DUAL; --2025-03-10 12:11:18.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-03-10 12:12:20.707 +0900
...
SELECT TO_CHAR(1000000, 'L9,999.999') || '원' FROM DUAL; -- '\1,000,0000'
SELECT TO_CHAR(1000000, 'FM9,999,999') || '원' FROM DUAL;
SELECT TO_CHAR(1000000, 'L999,999,999') || '원' FROM DUAL;
-- 날짜 -> 문자 변환시 포맷 패턴
-- YYYY : 년도 / YY : 년도(짧게)
-- MM : 월
-- DD : 일
...
-- 2) 추출한 생년월일을 TO_DATE 타입으로 변경 -> R 패턴을 이용하여 1900 년대로 변경
SELECT EMP_NAME, 
TO_DATE(SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') -1 ),'RRMMDD') AS 생년월일
FROM EMPLOYEE;

-- 3) TO_CHAR 를 이용해서 문자열로 변환 -> 1962년 12월 31일
SELECT EMP_NAME, 
...
-- NULL 처리 함수
-- NVL(컬럼명, 컬럼값이 NULL 일때 바꿀 값) : NULL 인 컬럼값을 다른 값으로 변경
...
-- 해당 컬럼의 값이 NULL 이면 바꿀값2로 변경
-- EMPLOYEE 테이블에서 보너스를 받으면 'O', 없으면 'x'  로 조회
...
-- DECODE(계산식 | 컬럼명, 조건값1, 선택값1, 조건값2, 조건값2, ..., 아무것도 일치하지 않을때)
...
CASE WHEN SALARY >= 5000000 THEN '대' -- IF
 		 ...
 		 ELSE '소' -- ELSE
 		 END "급여받는 정도"
 		 FROM EMPLOYEE;-- 급여가 300만원 미만이면 '소' 로 조회
...
SELECT SUM(SALARY), ROUND(AVG(SALARY)) - 3번째
FROM EMPLOYEE -- 1번째
WHERE DEPT_CODE = 'D9'; -- 2번째
...
-- 급여 최대값, 가장 최근 입사일, 알파벳 숫서가 가장 느린 이메일 조회
...
-- COUNT() : 행 개수를 헤아려서 반환
-- COUNT(컬럼명) : NULL 을 제외한 실제값이 기록된 행 개수를 리턴
-- COUNT(*) : NULL 을 포함한 전제 행 개수를 리턴
-- COUNT([DISTINCT] 컬럼명) : 중복을 제거한 행 개수를 리턴

SELECT COUNT(BONUS) FROM EMPLOYEE; -- 9
SELECT COUNT(*) FROM EMPLOYEE; -- 23
SELECT COUNT(DISTINCT JOB_CODE) FROM EMPLOYEE; -7
